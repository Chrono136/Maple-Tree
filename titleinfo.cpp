...
void TitleInfo::decryptContent(Decrypt *decrypt)
{
    ...
    if (!QFile(tmd).exists())
    {
        QMessageBox::critical(nullptr, "Error", "tmd not found, decryption failed:"+this->getDirectory());
        return;
    }
    if (!QFile(cetk).exists())
    {
        QMessageBox::critical(nullptr, "Error", "cetk not found, decryption failed:"+this->getDirectory());
        return;
    }
